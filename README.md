...
Автоматически полученные аннотации были сконвертированы из формата JSON в формат **XML Pascal VOC** для использования в CVAT.
...
3. **Запустите контейнер:**
    ```bash
    docker run -p 8000:8000 -d tbank-detector
    ```
...
1. Скачайте валидационные данные по ссылке выше и поместите их в папку `validation_data`.
...
- разнообразие выборки, в ней присутстовало большое колиечество схожих логотипов на желтом фоне в левом углу экрана.
Пример: ![Реазультаты обучения yolo](docs/01a937af199c598e137d310e5ae80951.jpg)
И довольно малеьнкое количество более "необычных" логотипов на зданиях, либо других объектах.

- Использование большего разрешения и модели YOLOv8m или OLOv8l
...
- Эксперементы с использованием модели для разметки другую модель, YOLO-World или Grounded-SAM